...
export default function MeetingUI({ outcome, biasActive }) {
  const isTargeted = biasActive && (outcome?.biasLevel || 0) > 20;

  const participants = [
    { id: 1, name: outcome?.speakerName || 'MARK', role: 'Director', color: '#00D4FF' },
  ...
  return (
    <div className="flex flex-col h-full bg-[#151928] border-2 border-white/5 rounded-xl overflow-hidden shadow-[inset_0_10px_30px_rgba(0,0,0,0.8)]">
      <div className="flex-1 p-2 grid grid-cols-2 gap-2 bg-[#0B0F1A]">
        {participants.map((participant, index) => (
          <motion.div
            ...
            className={`
              ...
              ${isTargeted && participant.id === 1 ? 'border-accent shadow-[0_0_20px_rgba(255,77,109,0.5),_0_4px_0_#900021]' : `border-[${participant.color}]10`}
            `}
          >
            <div className="text-4xl font-display font-black text-white/10" style={{ fontFamily: 'Fredoka' }}>
            ...
            <div className="absolute bottom-2 left-2 bg-black/80 px-3 py-1 rounded-md border border-white/10 flex items-center gap-2 shadow-game">
              <div className={`w-2 h-2 rounded-full ${isTargeted && participant.id === 1 ? 'bg-accent animate-pulse' : 'bg-green-400'}`} />
              <div>
                <div className="text-[10px] font-display font-bold text-white uppercase">{participant.name}</div>
                <div className="text-[8px] font-display text-white/50 uppercase tracking-widest">{participant.role}</div>
              </div>
            </div>

            {isTargeted && participant.id === 1 && (
              <div className="absolute top-2 right-2 bg-accent text-white px-2 py-1 rounded text-[10px] uppercase font-black font-display animate-pulse shadow-game border-2 border-[#900021]">
                {outcome?.speakerName ? `${outcome.speakerName} dominates` : 'Speaking Over You'}
              </div>
            )}
          </motion.div>
        ))}
        ...
        <motion.div
          ...
          className={`
            ...
            ${isTargeted ? 'border-white/10 grayscale filter arcade-glitch' : 'border-primary/50'}
          `}
        >
      ...
      {outcome?.quote && (
        <div className="px-4 py-2 bg-[#0B0F1A] border-t border-white/5 text-xs font-display font-bold text-white/70">
          "{outcome.quote}"
        </div>
      )}
      ...
      <div className="h-16 bg-[#1f2937] border-t-[3px] border-[#0B0F1A] flex items-center justify-center gap-4 px-4 shadow-[inset_0_4px_0_rgba(255,255,255,0.05)]">
        ...
        <button className={`w-10 h-10 rounded-full flex items-center justify-center text-white border-b-2 border-black/50 transition-all ${isTargeted ? 'bg-[#FF4D6D] border-[#900021] pointer-events-none' : 'bg-[#FF4D6D] border-[#900021] hover:translate-y-[1px] hover:border-b-[1px]'}`}>
          <MicOff size={18} />
        </button>
